	...
	if ($input->get->ordn) {
		...
		if (SalesOrderQuery::create()->orderExists($ordn)) {
			...
			if ($input->get->finish) {
				...
				$page->body = $config->twig->render('warehouse/packing/finish/print-form.twig', ['page' => $page, 'labelsession' => $labelsession, 'printers' => $whse_printers, 'whsesession' => $whsesession]);
				$whse_printers->clear();
